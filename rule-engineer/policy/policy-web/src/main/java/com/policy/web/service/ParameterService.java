...
public class ParameterService {
    ...
    private Set<Parameter> getParameters(Set<Integer> elementIds) {
        ...
        List<PolicyElement> engineElementList = this.policyElementManager.lambdaQuery().in(PolicyElement::getId, elementIds).list();
    ...
    private void getFromVariableElement(Set<Integer> elementIds, Value value) {
        if (value instanceof Variable) {
            Value val = this.PolicyConfiguration.getEngineVariable().getVariable(((Variable) value).getVariableId());
    ...
    private void conditionAllElementId(Set<Integer> elementIds, Integer type, String value) {
        if (VariableType.VARIABLE.getType().equals(type)) {
            Value val = this.PolicyConfiguration.getEngineVariable().getVariable(Integer.valueOf(value));
            if (val instanceof Function) {
                Function function = (Function) val;
                Map<String, Value> param = function.getParam();
                Collection<Value> values = param.values();
     ...
     * 获取条件中的元素
     */
    public Set<Parameter> getParameter(PolicyCondition policyCondition) {
        Set<Integer> elementIds = new HashSet<>();
        // 左边的
        this.conditionAllElementId(elementIds, policyCondition.getLeftType(), policyCondition.getLeftValue());
        // 右边的
        this.conditionAllElementId(elementIds, policyCondition.getRightType(), policyCondition.getRightValue());
        if (CollUtil.isEmpty(elementIds)) {
            return Collections.emptySet();
        }
        return this.getParameters(elementIds);
