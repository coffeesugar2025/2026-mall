...
import com.policy.common.enums.HtmlTemplatesEnum;
import com.policy.common.enums.VerifyCodeType;
import com.policy.common.exception.LoginException;
import com.policy.common.util.*;
import com.policy.web.config.AbstractTokenInterceptor;
import com.policy.web.config.AuthInterceptor;
import com.policy.web.config.Context;
import com.policy.web.service.RoleService;
import com.policy.web.service.UserService;
...
public class UserServiceImpl implements UserService {
    ...
    @Resource
    private PolicyUserManager policyUserManager;
    @Resource
    ...
    public boolean login(LoginRequest loginRequest) {
        PolicyUser policyUser = policyUserManager.lambdaQuery()
                .and(a -> a.eq(PolicyUser::getUsername, loginRequest.getUsername())
                        .or().eq(PolicyUser::getEmail, loginRequest.getUsername())).one();
        if (policyUser == null) {
            throw new LoginException("用户名/邮箱不存在!");
        }
//        if (!(PolicyUser.getPassword().equals(MD5Utils.encrypt(loginRequest.getPassword())))) {
//            throw new LoginException("登录密码错误!");
//        }
        String token = JWTUtils.genderToken(String.valueOf(policyUser.getId()), this.issuer, policyUser.getUsername());
        ...
        this.refreshUserData(token, policyUser);
        return true;
    }
     ...
     *
     * @param token      token
     * @param policyUser 用户信息
     */
    private void refreshUserData(String token, PolicyUser policyUser) {
        UserData userData = BasicConversion.INSTANCE.convert(policyUser);
        // 重新拉取用户角色信息
        List<PolicyRole> bootRoles = this.roleService.listRoleByUserId(policyUser.getId());
    ...
    public Boolean register(RegisterRequest registerRequest) {
        ...
        policyUser.setUsername(registerRequest.getUsername());
        policyUser.setPassword(MD5Utils.encrypt(registerRequest.getPassword()));
        policyUser.setEmail(registerRequest.getEmail());
        policyUserManager.save(policyUser);
        return true;
    }
    ...
    public Boolean verifyName(String username) {
        return null != policyUserManager.lambdaQuery()
                .eq(PolicyUser::getUsername, username)
                .one();
    }
    ...
    public Boolean updatePassword(ForgotRequest forgotRequest) {
        ...
        PolicyUser policyUser = new PolicyUser();
        policyUser.setPassword(MD5Utils.encrypt(forgotRequest.getPassword()));
        return policyUserManager.lambdaUpdate()
                .eq(PolicyUser::getEmail, forgotRequest.getEmail())
                .update(policyUser);
    }
    ...
    public Boolean updateUserInfo(UpdateUserInfoRequest userInfoRequest) {
        ...
        PolicyUser policyUser = this.policyUserManager.getById(userInfoRequest.getId());
        if (policyUser == null) {
            throw new ValidationException("没有此用户!");
        }
        policyUser.setId(userInfoRequest.getId());
        policyUser.setEmail(userInfoRequest.getEmail());
        policyUser.setPhone(userInfoRequest.getPhone());
        policyUser.setAvatar(userInfoRequest.getAvatar());
        policyUser.setSex(userInfoRequest.getSex());
        this.policyUserManager.updateById(policyUser);
        // 更新用户信息
