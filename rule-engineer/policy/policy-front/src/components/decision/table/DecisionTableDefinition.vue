    ...
    export default {
        ...
        data() {
            ...
            const validateIsExists = (rule, value, callback) => {
                ...
                this.$axios.post("/policy/decisionTable/codeIsExists",
                    {
                        "param": value
                    }).then(res => {
                    if (res.data) {
            ...
            nextStep() {
                ...
                this.$refs['form'].validate((valid) => {
                    if (valid) {
                        this.$axios.post("/policy/decisionTable/saveOrUpdateDecisionTableDefinition", {
                            "id": this.form.id,
                            "code": this.form.code,
                            "name": this.form.name,
            ...
            getDecisionTableDefinition() {
                ...
                this.loading = true;
                this.$axios.post("/policy/decisionTable/getDecisionTableDefinition", {
                    "id": this.form.id,
                }).then(res => {
                    let da = res.data;
                    if (da != null) {
