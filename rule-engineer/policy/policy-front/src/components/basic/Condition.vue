          ...
          <el-form-item label="左值" style="margin-top: 18px">
            <el-col :span="9">
              <el-form-item prop="config.leftValue.type">
                <el-select v-model="form.config.leftValue.type" placeholder="请选择数据类型"
                  ...
                  <el-option label="日期" :value="9" @click.native="form.config.leftValue.valueType='DATE'"/>
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="1">
            ...
            <el-col :span="9">
              <el-form-item prop="config.leftValue.value">
                ...
                <el-date-picker
                  v-else-if="form.config.leftValue.type===9" v-model="form.config.leftValue.value"
                  :disabled="form.config.leftValue.type==null"
                  type="datetime"
          ...
          <el-form-item label="右值" style="margin-top: 18px">
            <el-col :span="9">
              <el-form-item prop="config.rightValue.type">
                <el-select v-model="form.config.rightValue.type" placeholder="请选择数据类型"
                  ...
                  <el-option v-if="isRightTypeSelectView('DATE')" label="日期" :value="9"/>
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="1">
            ...
            <el-col :span="9">
              <el-form-item prop="config.rightValue.value">
                ...
                <el-date-picker
                  v-else-if="form.config.rightValue.type===9" v-model="form.config.rightValue.value"
                  :disabled="form.config.symbol==null"
                  type="datetime"
                  value-format="yyyy-MM-dd HH:mm:ss"
                  placeholder="选择日期时间"
      ...
      <el-form ref="runConditionForm" :rules="run.rules" :model="run.condition" label-width="80px">
        ...
        <el-form-item label="配置信息">
          <el-tag class="item" type="info" effect="plain" style="margin-top: 2px;">
            ...
            <span
              style="color: #606266">{{
                run.condition.config.leftValue.variableValue != null ? run.condition.config.leftValue.variableValue : run.condition.config.leftValue.valueName
              }}</span>

            &nbsp;
            <el-tag type="warning" style="height: 22px;line-height: 22px;padding: 0 2px 0 2px;">
              {{ $common.getSymbolExplanation(run.condition.config.symbol) }}
            </el-tag>
            ...
            <span
              style="color: #606266"> {{
                run.condition.config.rightValue.variableValue != null ? run.condition.config.rightValue.variableValue : run.condition.config.rightValue.valueName
              }}</span>
          </el-tag>
        </el-form-item>
...
export default {
  ...
  data() {
    const validateIsExists = (rule, value, callback) => {
      ...
      this.$axios.post("/policy/condition/nameIsExists",
        {
          "param": value
        }).then(res => {
        if (res.data) {
  ...
  methods: {
    ...
    exe() {
      this.$axios.post("/policy/condition/run", {
    ...
    test(row) {
      ...
      this.run.condition.output = null;
      this.$axios.post("/policy/condition/getParameter", {
        "id": row.id
      }).then(res => {
    ...
    saveOrUpdate(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$axios.post(this.form.id === null ? "/policy/condition/add" : "/policy/condition/update", {
            "id": this.form.id,
            "name": this.form.name,
    ...
    list() {
      this.$axios.post("/policy/condition/list", {
        "page": {
    ...
    leftRemoteMethod(query) {
      if (query !== '') {
        ...
        this.$axios.post(type === 1 ? "/policy/variable/list" : "/policy/element/list", {
          "page": {
            "pageSize": 10,
            "pageIndex": 1
          },
    ...
    edit(row) {
      this.clearValidate();
      this.$axios.post("/policy/condition/get", {
        "id": row.id
      }).then(res => {
        let da = res.data;
        if (da != null) {
    ...
    deleteRow(row) {
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        this.$axios.post("/policy/condition/delete", {
          "id": row.id
        }).then(res => {
          let da = res.data;
          if (da) {
