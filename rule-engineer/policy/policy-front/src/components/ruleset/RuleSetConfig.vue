                      ...
                      <div>
                        <el-form-item :prop="'ruleSet.' + index + '.action.type'" class="el-col-6"
                          ...
                          <el-select v-model="rs.action.type" @change="actionTypeChange(rs)">
                            ...
                            <el-option label="日期" :value="6" @click.native="rs.action.valueType='DATE'"/>
                          </el-select>
                        </el-form-item>
                  ...
                  <el-form-item prop="action.type" class="el-col-6"
                    ...
                    <el-select v-model="defaultRule.action.type"
                      ...
                      <el-option label="日期" :value="6"
                                 @click.native="defaultRule.action.valueType='DATE'"/>
                    </el-select>
                  </el-form-item>
                  <el-form-item class="el-col-1">
                    &nbsp;
...
export default {
  ...
  methods: {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          this.$refs["defaultRule"].validate((valid) => {
            if (valid) {
              this.$axios.post("/policy/ruleSet/generationRelease", {
                "id": this.id,
    ...
    update() {
      this.$axios.post("/policy/ruleSet/updateRuleSet", {
        "id": this.id,
        "ruleSet": Array.from(this.form.ruleSet).map(m => ({
          id: m.id,
          uuid: m.uuid,
    ...
    getRuleSetConfig() {
      this.loading = true;
      this.$axios.post("/policy/ruleSet/getRuleSetConfig", {
        "id": this.id
      }).then(res => {
        let da = res.data;
        if (da != null) {
