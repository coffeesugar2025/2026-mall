    ...
    export default {
        ...
        methods: {
            ...
            run() {
                ...
                this.$axios.post("/policy/ruleSetTest/run", requestJson).then(res => {
                    let da = res.data;
                    if (da != null) {
            ...
            publish() {
                this.$confirm('此操作将会改变线上规则集运行, 是否继续?', '提示', {
                    ...
                    this.$axios.post("/policy/ruleSet/publish",
                        {
                            id: this.id
                        }
                    ).then(res => {
            ...
            getRuleSetView(id) {
                this.loading = true;
                this.$axios.post("/policy/ruleSet/view", {
                    "id": id,
                    "status": 1
                }).then(res => {
                    let da = res.data;
