    ...
    export default {
        ...
        data() {
            ...
            const validateIsExists = (rule, value, callback) => {
                ...
                this.$axios.post("/policy/ruleSet/codeIsExists",
                    {
                        "param": value
                    }).then(res => {
                    if (res.data) {
        ...
        methods: {
            nextStep() {
                ...
                this.$refs['form'].validate((valid) => {
                    if (valid) {
                        this.$axios.post("/policy/ruleSet/saveOrUpdateRuleSetDefinition", {
                            "id": this.form.id,
                            "code": this.form.code,
                            "name": this.form.name,
            ...
            getRuleSetDefinition() {
                ...
                this.loading = true;
                this.$axios.post("/policy/ruleSet/getRuleSetDefinition", {
                    "id": this.form.id,
                }).then(res => {
                    let da = res.data;
                    if (da != null) {
