    ...
    export default {
        ...
        data() {
            ...
            const validateIsExists = (rule, value, callback) => {
                ...
                this.$axios.post("/policy/element/codeIsExists",
                    {
                        "param": value
                    }).then(res => {
                    if (res.data) {
            ...
            saveOrUpdateClick(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$axios.post(this.form.id === null ? "/policy/element/add" : "/policy/element/update", this.form)
                            .then(res => {
                                if (res) {
                                    this.list();
                                }
            ...
            list() {
                this.$axios.post("/policy/element/list", {
                    "page": {
            ...
            edit(row) {
                this.clearValidate();
                this.$axios.post("/policy/element/get", {
                    "id": row.id
                }).then(res => {
                    let da = res.data;
                    if (da != null) {
            ...
            deleteRow(row) {
                this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
                    ...
                    this.$axios.post("/policy/element/delete", {
                        "id": row.id
                    }).then(res => {
                        let da = res.data;
                        if (da) {
