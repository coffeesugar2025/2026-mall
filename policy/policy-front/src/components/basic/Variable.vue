          ...
          <el-col :span="4" style="margin-top: 26px">
            <el-form-item v-for="(pv,index) in form.function.paramValues" style="margin-top: 18px;"
              ...
              <el-select v-model="pv.type" @change="leftValueTypeChange(pv)">
                ...
                <el-option label="日期" :value="9" v-else-if="pv.valueType==='DATE'"/>
              </el-select>
            </el-form-item>
          </el-col>
...
export default {
  ...
  data() {
    const validateIsExists = (rule, value, callback) => {
      ...
      this.$axios.post("/policy/variable/nameIsExists",
        {
          "param": value
        }).then(res => {
        if (res.data) {
          callback(new Error('变量名称已经存在'));
        } else {
          callback();
        }
      });
    ...
    leftRemoteMethod(query, pv) {
      if (query !== '') {
        pv.loading = true;
        pv.options = [];
        this.$axios.post(pv.type === 1 ? "/policy/variable/list" : "/policy/element/list", {
          "page": {
            "pageSize": 10,
            "pageIndex": 1
          },
    ...
    remoteMethod(query) {
      if (query !== '') {
        ...
        this.form.options = [];
        this.$axios.post("/policy/function/list", {
          "page": {
            "pageSize": 10,
            "pageIndex": 1
          },
    ...
    saveOrUpdate(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$axios.post(this.form.id != null ? "/policy/variable/update" : "/policy/variable/add", {
            "id": this.form.id,
            "name": this.form.name,
    ...
    list() {
      this.loading = true;
      this.$axios.post("/policy/variable/list", {
        "page": {
    ...
    getVarInfo(id) {
      ...
      this.form.id = id;
      // 查询到变量信息
      this.$axios.post("/policy/variable/get", {
        "id": id
      }).then(res => {
        let da = res.data;
        if (da != null) {
    ...
    deleteRow(row) {
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        this.$axios.post("/policy/variable/delete", {
          "id": row.id
        }).then(res => {
          let da = res.data;
          if (da) {
