                  ...
                  <el-timeline-item
                    ...
                    <div v-for="(c,ci) in cg.conditionGroupCondition" style="margin-left: 20px;">
                      <el-tag class="item" type="info" effect="plain" style="margin-top: 2px">
                        ...
                        <span
                          style="color: #606266">{{ c.condition.config.leftValue.variableValue != null ? c.condition.config.leftValue.variableValue : c.condition.config.leftValue.valueName }}</span>
                        ...
                        <span
                          style="color: #606266"> {{ c.condition.config.rightValue.variableValue != null ? c.condition.config.rightValue.variableValue : c.condition.config.rightValue.valueName }}</span>
                      </el-tag>
...
export default {
  ...
  data() {
    ...
    run() {
      ...
      this.$axios.post("/policy/generalRuleTest/run", requestJson).then(res => {
        let da = res.data;
        if (da != null) {
    ...
    publish() {
      this.$confirm('此操作将会改变线上规则运行, 是否继续?', '提示', {
        ...
        this.$axios.post("/policy/generalRule/publish",
          {
            id: this.id
          }
        ).then(res => {
    ...
    getRuleView(id) {
      this.loading = true;
      this.$axios.post("/policy/generalRule/view", {
        "id": id,
        "status": 1
      }).then(res => {
        let da = res.data;
