                ...
                <el-form ref="actionForm" :model="action" :rules="actionRules">
                  <el-form-item prop="type" class="el-col-6">
                    <el-select v-model="action.type" @change="actionTypeChange()">
                      ...
                      <el-option label="日期" :value="6" @click.native="action.valueType='DATE'"/>
                    </el-select>
                  </el-form-item>
              ...
              <div>
                <el-form ref="defaultAction" :model="defaultAction">
                  ...
                  <el-form-item prop="valueType" class="el-col-6"
                                :rules="defaultAction.enableDefaultAction===0? {required: true, message: '请选择结果类型', trigger: 'blur'}:{required: false}">
                    <el-select v-model="defaultAction.type"
                      ...
                      <el-option label="日期" :value="6"
                                 @click.native="defaultAction.valueType='DATE'"/>
                    </el-select>
                  </el-form-item>
                  <el-form-item class="el-col-1">
                    &nbsp;
                  ...
                  <el-form-item prop="value" class="el-col-17"
                                :rules="defaultAction.enableDefaultAction===0?{required: true, message: '请输入结果值', trigger: 'blur'}:{required: false}">
...
export default {
  ...
  methods: {
    ...
    update() {
      this.$axios.post("/policy/generalRule/updateRule", {
        "id": this.id,
      ...
      this.$refs["actionForm"].validate((valid) => {
        if (valid) {
          this.$refs["defaultAction"].validate((valid) => {
            if (valid) {
              this.$axios.post("/policy/generalRule/generationRelease", {
                "id": this.id,
                "conditionGroup": this.conditionGroup,
                "action": {
                  "value": this.action.value,
    ...
    getRuleConfig() {
      this.loading = true;
      this.$axios.post("/policy/generalRule/getRuleConfig", {
        "id": this.id
      }).then(res => {
        let da = res.data;
        if (da != null) {
